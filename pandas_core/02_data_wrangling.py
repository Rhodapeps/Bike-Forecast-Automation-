...
# Moving columns to the front (DataFrame Selection)
...
df.select_dtypes(exclude = ['datetime']).sum()
...
df.groupby('city')[df.select_dtypes(exclude=['datetime']).columns].sum()
...
summary_df_3 = df[["category_2","order_date","total_price","quantity"]] \
    .groupby(["category_2", pd.Grouper(key='order_date', freq='W')]) \
    .agg(np.sum) \
    .reset_index()
...
bikeshop_revenue_wide_df \
    ...
    .to_excel("pandas_core/bikeshop_revenue_wide.xlsx")



# Melt (Pivoting Longer)

bikeshop_revenue_long_df = pd.read_excel("pandas_core/bikeshop_revenue_wide.xlsx") \
    .iloc[:, 1:] \
    .melt(
...
sales_cat2_daily_df = df[['category_2', 'order_date', 'total_price']] \
    ...
    .resample('D') \
    .sum() \
    .drop(columns=['category_2'])


sales_cat2_daily_df

np.mean([1, 2, 3]) # Aggregration
